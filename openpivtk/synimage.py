# This module contains a synthetic piv image generator for validation purposes
...
class continuous_flow_field:
    def __init__(self,data,inter=False):
        ...
        self.inter = inter
        if inter:
            self.f_dU = scipy.interpolate.interp2d(data[:,0],data[:,1],data[:,2])
            self.f_dV = scipy.interpolate.interp2d(data[:,0],data[:,1],data[:,3])
    ...
    def get_U_V(self,x,y):
        ...
        if self.inter:
            return self.f_dU(x,y)[0],self.f_dV(x,y)[0]
        else:
            return self.f_U(x,y),self.f_V(x,y)
        
    def create_syn_quiver(self, nx, ny, xmax, ymax, path=None):
        ...
        fig.set_clim(vmin=1.5, vmax=m.max())
